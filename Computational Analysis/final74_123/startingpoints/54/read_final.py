...
for line in sol.readlines():
    ...
    if current_list == []:
        ...
        if variable_category == 'x':
            current_value_list = re.split(r'[+-]',current_list[2])
...
for i in J:
    if i in TN_comm_nodes:
        building_cost += theta * alpha*A_dict[i]*(1-Z_dict[i])/1000
        capacity_cost += abs(theta * alpha*C_dict[i]*X_dict[i]/1000)
    else:
        building_cost += theta * alpha*A_dict[i]*(1-Z_dict[i])/1000
        capacity_cost +=  abs(theta * alpha*C_dict[i]*X_dict[i]/1000)
...
print('OBJ_Manual:', (total_cost - (total_utility - unsatisified_utility - total_penalty )/n))
